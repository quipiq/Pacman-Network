...
def connexion():
    ...
    while 1:
        ...
        # Dialogue avec le client :
        nbr_joueur = len(list(conn_client.keys()))
        dernier_clients = list(conn_client.keys())
        numeros_clients = f"def {dernier_clients[-1]}"
        connexion.send(numeros_clients.encode("Utf8"))
        time.sleep(0.3)
        affichage_nbr_joueur = f"nbr_joueur {nbr_joueur}/4"
        for cle in conn_client:
             conn_client[cle].send(affichage_nbr_joueur.encode("Utf8"))
        if nbr_joueur < 4:
            pass
        else:
            ...
            for cle in conn_client:
                lancement = f"START"
                conn_client[cle].send(lancement.encode("Utf8"))
